...
export default function NoweZamowienie() {
  ...
  const [formData, setFormData] = useState({
    ...
    time: '',
    availability: '',
    status: 'contacted', // Zlecenie = już skontaktowano się
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('📤 Wysyłanie danych zlecenia:', submitData);

      // Zmiana: używamy /api/rezerwacje które automatycznie tworzy klienta + zlecenie
      // Ale ustawiamy status na 'contacted' zamiast 'pending'
      submitData.status = 'contacted'; // Zlecenie już kontaktowane
      ...
      if (response.ok) {
        ...
        console.log('✅ Zlecenie utworzone:', result);
        
        // Przekieruj do listy zamówień
        router.push('/admin/zamowienia');
      } else {
        const error = await response.json();
        alert(`Błąd: ${error.message || 'Nie udało się utworzyć zlecenia'}`);
      }
    } catch (error) {
