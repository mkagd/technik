...
export default function TechnicianMojMagazyn() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setEmployee(emp);
      loadInventory(emp.id, token);
    } catch (err) {
      router.push('/technician/login');
    }
  }, []);

  const loadInventory = async (employeeId, token) => {
    setLoading(true);
    try {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              <Link href="/technician/magazyn" className="text-blue-600 hover:text-blue-700 text-sm font-medium">
                ← Wróć
              </Link>
            </div>
          </div>
