...
export default function TechnicianMojeZamowienia() {
  ...
  useEffect(() => {
    if (employee) {
      const token = localStorage.getItem('technicianToken');
      loadRequests(employee.id, token);
    }
  }, [filter]);

  const loadRequests = async (employeeId, token) => {
    setLoading(true);
    try {
