    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          ...
          {/* Zachęta do większej liczby godzin */}
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <h3 className="text-sm font-medium text-green-800">
                  Więcej godzin dyżuru = Więcej zleceń = Większy zarobek!
                </h3>
                <p className="text-sm text-green-700 mt-1">
                  Im więcej godzin ustawisz jako dostępne, tym więcej klientów będzie mogło umówić się na serwis. 
                  Większa dostępność oznacza więcej zleceń dziennie i wyższe miesięczne zarobki. 
                  <span className="font-medium">Każda dodatkowa godzina to potencjalna dodatkowa usługa!</span>
                </p>
              </div>
            </div>
          </div>
        ...
        {/* Tygodniowe podsumowanie z zachętą */}
        <div className="bg-white rounded-lg shadow p-6 mt-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Twoje tygodniowe możliwości</h3>
            ...
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              <div className="text-2xl font-bold text-blue-600">
                {daysOfWeek.reduce((total, day) => {
                  ...
                  const breakSlots = daySchedule.breaks ? daySchedule.breaks.length : 0;
                  const netWorkSlots = workSlots - breakSlots;
                  return total + ((netWorkSlots * 30) / 60);
                }, 0).toFixed(1)}h
              </div>
              <div className="text-sm text-blue-700 font-medium">Godziny dyżuru w tygodniu</div>
            </div>
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <div className="text-2xl font-bold text-green-600">
                {Math.floor(daysOfWeek.reduce((total, day) => {
                  const daySchedule = workSchedule[day.key] || { enabled: false, slots: [], breaks: [] };
                  const workSlots = daySchedule.slots ? daySchedule.slots.length : 0;
                  const breakSlots = daySchedule.breaks ? daySchedule.breaks.length : 0;
                  const netWorkSlots = workSlots - breakSlots;
              ...
              <div className="text-sm text-green-700 font-medium">Potencjalnych zleceń/tydzień</div>
              <div className="text-xs text-green-600 mt-1">Przy 2 zleceniach/godz</div>
            </div>
            <div className="text-center p-4 bg-emerald-50 rounded-lg">
              <div className="text-2xl font-bold text-emerald-600">
                {daysOfWeek.filter(day => (workSchedule[day.key] || {}).enabled).length}/7
              </div>
              <div className="text-sm text-emerald-700 font-medium">Dostępnych dni w tygodniu</div>
            </div>
          </div>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  ...
                  <p className="text-sm text-yellow-700">
                    Masz obecnie tylko {daysOfWeek.reduce((total, day) => {
                      const daySchedule = workSchedule[day.key] || { enabled: false, slots: [], breaks: [] };
                      const workSlots = daySchedule.slots ? daySchedule.slots.length : 0;
                      const breakSlots = daySchedule.breaks ? daySchedule.breaks.length : 0;
                      const netWorkSlots = workSlots - breakSlots;
                      return total + ((netWorkSlots * 30) / 60);
                    }, 0).toFixed(1)}h dyżuru w tygodniu. 
                    <span className="font-medium">Dodaj więcej godzin</span>, aby otrzymywać więcej zleceń 
                    i zwiększyć swoje miesięczne zarobki!
                  </p>
                </div>
              </div>
            </div>
